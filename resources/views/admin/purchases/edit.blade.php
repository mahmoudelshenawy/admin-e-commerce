...
<div class="col-12">
    <div class="card">
        ...
        <h4 class="card-title">{{ trans('admin.edit_purchase') }}</h4>                                
        </div>
                    ...
                    <div class="col-12">
                        ...
                        <form action="{{ route('purchases.update' , $purchase->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            <?php $admins = \App\Admin::all(['id' , 'name']); ?>                                         
                                ...
                                <option value="{{$admin->id}}" {{$purchase->admin_id == $admin->id ? 'selected' : ''}}>{{$admin->name}}</option>
                                @empty
                            ...
                            <div class="form-group col-md-6 col-sm-12">
                                ...
                                <option value="{{$product->id}}" {{$purchase->product_id == $product->id ? 'selected' : ''}}>{{$product->title}}</option>
                                @empty
                                    <option>NO data available</option>
                                @endforelse
                                    </select>
                                ...
                                @enderror 
                                    ...
                                    name="quantity" placeholder="{{trans('admin.quantity')}}" value="{{$purchase->quantity}}">
                                    @error('quantity')
                                    ...
                                    @enderror
                                        ...
                                        <option value="{{$item}}" {{$purchase->status == $item ? 'selected' : ''}}>{{$item}}</option>
                                            @endforeach
                                        </select>
                                        @error('status')
                                        <span class="text-danger">{{ $message }}</span>
                                    ...
                                    name="purchase_price" placeholder="{{trans('admin.purchase_price')}}" value="{{$purchase->purchase_price}}">
                                 ...
                                 @enderror 
                                    ...
                                    name="discount" placeholder="{{trans('admin.discount')}}" value="{{$purchase->discount}}">
                                    @error('discount')
                                    <span class="text-danger">{{ $message }}</span>
                                 @enderror 
                                        </div>
                                    ...
                                    name="tax" placeholder="{{trans('admin.tax')}}" value="{{$purchase->tax}}">
                                    @error('tax')
                                    <span class="text-danger">{{ $message }}</span>
                                 @enderror 
                                        </div>
                                        ...
                                        name="coupon" placeholder="{{trans('admin.coupon')}}" value="{{$purchase->coupon}}">
                                        @error('coupon')
                                        <span class="text-danger">{{ $message }}</span>
                                     @enderror 
                                            </div>
                                        ...
                                        <div class="row d-flex justify-content-center">
                                            ...
                                            <div class="col md-6 col-6">
                                            <h3 class="total_price">{{$purchase->total_price}}</h3>
                                            <input type="hidden" name="total_price" value="{{$purchase->total_price}}" id="sent_total_price">
                                            </div>
                                        </div>
                                        <hr>
                                        <br>
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-4 col-sm-6">
                                                <label for="payment_type">{{trans('admin.payment_type')}}</label>                  
                                                    ...
                                                    <option value="{{$item}}" {{$purchase->payment_type == $item ? 'selected' : ''}}>{{$item}}</option>
                                                        @endforeach
                                                    </select>
                                                    @error('payment_type')
                                                    <span class="text-danger">{{ $message }}</span>
                                                ...
                                                <div class="form-group col-md-4 col-sm-6">
                                                    ...
                                                    name="payment_price" placeholder="{{trans('admin.payment_price')}}" value="{{$purchase->payment_price}}">  
                                                    @error('payment_price')
                                                    <span class="text-danger">{{ $message }}</span>
                                                 @enderror   
                                                </div> 
                                                <div class="form-group col-md-4 col-sm-6">
                                                    <label for="payment_status">{{trans('admin.payment_status')}}</label>                  
                                                        ...
                                                        <option value="{{$item}}" {{$purchase->payment_status == $item ? 'selected' : ''}}>{{$item}}</option>
                                                            @endforeach
                                                        </select>
...
<div class="row">
    ...
    <input type="date" class="form-control datepickerInput" name="submit_time" id="submit_time" value="{{$purchase->submit_time}}">
        @error('submit_time')
        <span class="text-danger">{{ $message }}</span>
     @enderror 
    </div>
    ...
    <input type="date" class="form-control datepickerInput" name="delivery_time" id="delivery_time" value="{{$purchase->delivery_time}}">
        @error('delivery_time')
        <span class="text-danger">{{ $message }}</span>
     @enderror 
    </div>
                            ...
                            <div class="form-group">                                                  
                            <button type="submit" class="btn btn-primary">{{ trans('admin.edit_purchase') }}</button></button>
                            </div>
                        </form>
                    </div>
                </div>
