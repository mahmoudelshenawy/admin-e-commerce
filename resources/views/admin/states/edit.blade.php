                    ...
                    <div class="col-12">
                        <form action="{{ route('states.update', $state->id) }}" method="post">
                            ...
                            <label for="username1">{{trans('admin.state_name_ar')}}</label>                                                  
                                <input type="text" class="form-control" id="username1" name="state_name_ar" placeholder="{{trans('admin.state_name_ar')}}" value={{$state->state_name_ar}}>                                                    
                            </div>
                            <div class="form-group">
                                <label for="username2">{{trans('admin.state_name_en')}}</label>                                                  
                                    <input type="text" class="form-control" id="username2" name="state_name_en" placeholder="{{trans('admin.state_name_en')}}" value={{$state->state_name_en}}>                                                    
                                </div>
                                <div class="form-group">
                                    <label for="select">{{__('admin.country')}}</label>
                                   <select name="country_id" id="select" class="form-control country_id">
                                      
                                       <option>........</option>
                                       @foreach ($countries as $country)
                                   <option value="{{$country->id}}" @if ($state->country_id == $country->id)
                                    {{ 'selected' }}
                                   @endif>
                                ...
                                <div class="form-group">
                                 ...
                                 <select name="city_id" id="city" class="form-control city">
                                     ...
                                     <option value="{{$city->id}}" @if ($state->city_id == $city->id)
                                      {{ 'selected' }}
                                     @endif>
                                      {{session('lang') == 'ar' ? $city->city_name_ar : $city->city_name_en }}</option>
                                         @endforeach
                                 </select>
                                </div>
                           
                            <div class="form-group">                                                  
                            <button type="submit" class="btn btn-primary">{{ trans('admin.edit_state') }}</button></button>
                            </div>
                        </form>
                    </div>
                </div>
